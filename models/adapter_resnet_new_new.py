...
class ResNet_New(nn.Module):
    ...
    def _make_layer(self, block, blocks_num, stride):
        ...
        self.layer_num += 1
        # 在第1， 3， 5， 7个block后面插入adapter结构，从第0个开始
        need_adapter = [1, 3, 5, 7]
        for i in range(1, blocks_num):
            layers.append(block(self.mid_channel[self.layer_num - 1], self.overall_channel[self.layer_num - 1],
                                     self.overall_channel[self.layer_num]))
            # 添加adapter结构
            if i in need_adapter:
                layers.append(Adapter3(self.overall_channel[self.layer_num], self.adapter_channel[self.adapter_layer]))
...
def adapter8resnet_56(sparsity, num_classes, adapter_sparsity):
