...
from data import cifar10, cifar100, cub, tinyimagenet, svhn, dtd, \
    mnist, size224_cifar10, size224_cifar100, imagenet
import torch
...
def dstget(args):
    dsetlist = ['cifar10', 'cifar100', 'cub', 'tinyimagenet', 'svhn', 'dtd', 'mnist', 'cifar10224', 'cifar100224', 'imagenet']
    dldfunlist = [cifar10.load_cifar_data, cifar100.load_cifar_data, cub.load_cub_data, tinyimagenet.load_tinyimagenet_data,
                  svhn.load_svhn_data, dtd.load_dtd_data, mnist.load_mnist_data, size224_cifar10.load_cifar_data,
                  size224_cifar100.load_cifar_data, imagenet.load_imagenet]
...
def graf_load_adapter25resnet_model(args, model, oristate_dict, layer, logger, name_base=''):
    cfg = {
        20: [3, 3, 3],
        56: [9, 9, 9],
        110: [18, 18, 18],
...
def graf_overall_load_adapter_efficientnet_model(args, model, oristate_dict, layer, logger, name_base=''):
    state_dict = model.state_dict()
...
def graf_overall_load_adapter_efficientnet_v2_model(args, model, oristate_dict, layer, logger, name_base=''):
    state_dict = model.state_dict()
    ...
    l = 1
    # group_conv_list = [2, 7, 12, 17, 22, 27, 32, 37, 42, 47, 52, 57, 62, 71]
    group_conv_list = [2, 7, 12, 17, 22, 27, 32, 37, 42, 47, 52, 57, 62, 67]
    for ii in range(16):
        if ii == 0:
            t_list = [(0, 0), (1, 'conv', 0), (1, 'conv', 2), (2, 0)]
        elif ii == 14:
...
def overall_load_adapter_efficientnet_v2_model(args, model, oristate_dict, layer, logger, name_base=''):
    state_dict = model.state_dict()
...
def overall_load_adapter_efficientnet_v5_model(args, model, oristate_dict, layer, logger, name_base=''):
    state_dict = model.state_dict()
    ...
    # group_conv_list = [2, 7, 12, 17, 22, 27, 32, 37, 42, 47, 52, 57, 62, 71]
    # group_conv_list = [2, 7, 12, 17, 22, 27, 32, 37, 42, 47, 52, 57, 62, 67]
    group_conv_list = [2, 7, 12, 17, 22, 27, 32, 37, 42, 47, 52, 57, 66]
    for ii in range(15):
        if ii == 0:
            t_list = [(0, 0), (1, 'conv', 0), (1, 'conv', 2), (2, 0)]
        elif ii == 12:
...
def graf_overall_load_adapter_efficientnet_v5_model(args, model, oristate_dict, layer, logger, name_base=''):
    state_dict = model.state_dict()
...
def overall_load_efficientnet_model(args, model, oristate_dict, layer, logger, name_base=''):
    state_dict = model.state_dict()
...
def load_arch_model(args, model, origin_model, ckpt, logger, graf=False):
    ...
    if graf == False:
        ...
        if args.arch == 'vgg_16_bn':
            ...
            overall_load_efficientnet_model(args, model, oristate_dict, 81, logger)
        elif args.finetune_arch == 'adapter_efficientnet_b0_changed_v4_v2':
            logger.info('load adapter_efficientnet_v2_model')
            overall_load_adapter_efficientnet_v2_model(args, model, oristate_dict, 81, logger)
        elif args.finetune_arch == 'adapter_efficientnet_b0_changed_v5':
            logger.info('load adapter_efficientnet_v5_model')
            overall_load_adapter_efficientnet_v5_model(args, model, oristate_dict, 81, logger)
            ...
            load_resnet_model(args, model, oristate_dict, 56, logger)
        elif args.arch == 'adapter25resnet_56':
            logger.info('load adapter25 resnet 56 model')
            load_resnet_model(args, model, oristate_dict, 56, logger)
    ...
    # 在不同的模型或者不同的数据集上进行裁剪
        ...
        if args.pretrained_arch == args.finetune_arch:
            if args.finetune_arch == 'resnet_20':
                ...
                logger.info('graf_load_adapter_efficientnet_b0_changed_v4_model')
                graf_overall_load_adapter_efficientnet_v2_model(args, model, oristate_dict, 81, logger)
            elif args.finetune_arch == 'adapter_efficientnet_b0_changed_v5':
                logger.info('graf_load_adapter_efficientnet_b0_changed_v5_model')
                graf_overall_load_adapter_efficientnet_v5_model(args, model, oristate_dict, 81, logger)
            ...
            elif args.finetune_arch == 'adapter25resnet_56':
                logger.info('graf_load_adapter25resnet_model')
                graf_load_adapter25resnet_model(args, model, oristate_dict, 56, logger)
